...
if [ $fold == 0 ]; then
  # ! model path
  model=/data/duongdb/FH_OCT_08172021/Stylegan2/00001-Tf256RmFold0+EyePos+FH-paper256-kimg3000-ada-target0.8-resumeffhq256-divlabel4/network-snapshot-000409.pkl
fi

if [ $fold == 1 ]; then
  model=/data/duongdb/FH_OCT_08172021/Stylegan2/00002-Tf256RmFold1+EyePos+FH-paper256-kimg3000-ada-target0.8-resumeffhq256-divlabel4/network-snapshot-000461.pkl
fi

if [ $fold == 2 ]; then
  model=/data/duongdb/FH_OCT_08172021/Stylegan2/00004-Tf256RmFold2+EyePos+FH-paper256-kimg3000-ada-target0.8-resumeffhq256-divlabel4/network-snapshot-000409.pkl
fi

if [ $fold == 3 ]; then
  model=/data/duongdb/FH_OCT_08172021/Stylegan2/00000-Tf256RmFold3+EyePos+FH-paper256-kimg3000-ada-target0.8-resumeffhq256-divlabel4/network-snapshot-000358.pkl
fi

if [ $fold == 4 ]; then
  model=/data/duongdb/FH_OCT_08172021/Stylegan2/00003-Tf256RmFold4+EyePos+FH-paper256-kimg3000-ada-target0.8-resumeffhq256-divlabel4/network-snapshot-000461.pkl
fi


truncationpsi=TRUNCATION # @trunc set at what ???
...
labelset_head = sorted('1,2,3,4'.split(','))
...
def make_label_pairs (name_array1,name_array2,labelset_head_in_gan,labelset_tail_in_gan): 
  ...
  for n1 in name_array1: # condition on "fixed", this is "eye" because eyes don't change 
    for n2 in list(itertools.combinations(name_array2, 2)): # @n2 is array of tuple [(disease1,disease2)...]
      if ('1' in n2 and '2' in n2) or ('2' in n2 and '3' in n2) or ('3' in n2 and '4' in n2): 
...
#
rootout = '/data/duongdb/FH_OCT_08172021/Classify/'+name_option+'Soft+FH+Eye'+'M'+str(MIXRATIO)+'T'+str(TRUNCATION)
...
for fold in [0,1,2,3,4]: # ,2,3,4,5
  ...
  for selectlabel in label_pair_key: 
    ...
    #
    if label1 in ['1OD', '1OS', '4OD', '4OS'] : 
      imglist = np.arange ( classifier_train_csv.shape[0] )
    else: 
      imglist = np.arange ( classifier_train_csv.shape[0]//2 ) # ! don't need this "//2" ?? we don't have A->B A->C, so we double, to keep same size, we need //2
    ...
    time.sleep(2)
    # break
    os.system('sbatch --partition=gpu --time=00:20:00 --gres=gpu:k80:1 --mem=8g --cpus-per-task=4 '+fname)
