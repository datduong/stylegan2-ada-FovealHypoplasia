...
def create_from_images(tfrecord_dir, image_dir, shuffle, resolution=None, classifier_train_csv=None, fold=None, selectlabel=None):
    ...
    if classifier_train_csv is not None: 
        # remove the fold. 
        classifier_train_csv = pd.read_csv ( classifier_train_csv, dtype=str ) 
        fold = list ( set ( fold.split(',') ) ) # ! take many folds
        classifier_train_csv = classifier_train_csv[~classifier_train_csv['fold'].isin(fold)] # ! remove fold we don't need
...
def create_from_images_with_labels_fromjson (tfrecord_dir, image_dir, shuffle, labeljson_name='dataset.json', resolution=None, oversample_label=None, oversample_time=None, classifier_train_csv=None, fold=None, use_these_label=None):
    ...
    if classifier_train_csv is not None: 
        ...
        remove_fold = list ( set ( [5,fold] ) ) # ! remove both test set fold=5 and @fold. 
        print ('num ob before remove fold {} , {}'.format(remove_fold, classifier_train_csv.shape[0]))
        for f in remove_fold: 
            classifier_train_csv = classifier_train_csv[ classifier_train_csv['fold'] != f ]
        print ('num ob after remove fold {} , {}'.format(remove_fold, classifier_train_csv.shape[0]))
...
def execute_cmdline(argv):
    ...
    p.add_argument(     '--fold',      help='fold', type=str, default=None)
